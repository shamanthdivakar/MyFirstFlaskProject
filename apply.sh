...
original_name="myfirstflaskproject"
original_urlname="MyFirstFlaskProject"
original_description="Awesome myfirstflaskproject created by shamanthdivakar" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/myfirstflaskproject "${name}"
...
then
    ...
    echo "then run 'myfirstflaskproject' or 'python -m myfirstflaskproject'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'myfirstflaskproject' or 'python -m myfirstflaskproject' or 'poetry run myfirstflaskproject'"
fi
